...
func TestLaxNames(t *testing.T) {
	...
	_, err := compiler.CompileSchemaBytes([]byte(recordFoo.AvroRecordSchema()), []byte(recordBar.AvroRecordSchema()))
	assert.Error(t, err)
	...
	program, err := compiler.CompileSchemaBytes(
		[]byte(recordFoo.AvroRecordSchema()),
		[]byte(recordBar.AvroRecordSchema()),
