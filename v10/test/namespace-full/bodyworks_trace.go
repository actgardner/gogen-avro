...
type BodyworksTrace struct {
	// Trace Identifier
	TraceId *HeaderworksDatatypeUUID `json:"traceId"`
}
...
func writeBodyworksTrace(r BodyworksTrace, w io.Writer) error {
	...
	if r.TraceId == nil {
		...
		err = writeHeaderworksDatatypeUUID(*r.TraceId, w)
	}
	return err
}
...
func (r *BodyworksTrace) Get(i int) types.Field {
	...
	case 0:
		if r.TraceId == nil {
			var TraceId = new(HeaderworksDatatypeUUID)
			r.TraceId = TraceId
		}
		w := r.TraceId
...
func (r *BodyworksTrace) UnmarshaltraceIdJSON(data []byte) error {
	...
	if v, ok := fields["headerworks.datatype.UUID"]; ok {
		r.TraceId = new(HeaderworksDatatypeUUID)
